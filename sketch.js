...
function setup() {
	...
	buttonRem = createButton('Rem');
	buttonRem.position(270, 0);
	buttonRem.mousePressed(function () {
		bilden = rem;
		skip = 30
		färg = [173, 216, 230];
		weight = 2;
	...
	buttonRita = createButton('Rita själv');
	buttonRita.position(320, 0);
	buttonRita.mousePressed(function () {
		ritaSjälv = true;
		skip = 1;
		färg = [255, 255, 255];
		weight=2;
	...
	buttonKlar = createButton('Du har ritat klart');
	buttonKlar.position(400, 0);
	buttonKlar.mousePressed(function () {
		körigång();
		ritaSjälv = false;
	})
